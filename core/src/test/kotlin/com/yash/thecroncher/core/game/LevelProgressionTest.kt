...
class LevelProgressionTest {
    ...
    /** Walks the croncher over every pellet in turn, which is quicker than playing. */
    private fun GameState.clearTheMaze() {
        for (pellet in maze.pelletPositions) {
            croncher.placeAtTileCentre(pellet, Direction.LEFT)
            tick()
    ...
    @Test
    fun `a level starts with all 364 pellets`() {
        val g = game()
        assertEquals(364, g.pelletsRemaining)
    ...
    fun `the next level restocks the maze and speeds things up`() {
        val g = game()
        val speedBefore = g.croncher.speed
        g.clearTheMaze()
    ...
    fun `an extra life arrives at ten thousand points`() {
        ...
        g.clearTheMaze()
        // 2,600 from pellets is not enough on its own, so top up via toy-free play.
