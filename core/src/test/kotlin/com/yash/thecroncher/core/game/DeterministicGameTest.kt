...
class DeterministicGameTest {
    ...
    private fun trajectoryHash(ticks: Int, seed: Long = 20260908L): Long {
        ...
        for (tick in 0 until ticks) {
            ...
            fold(game.croncher.x.toLong() * 31 + game.croncher.y)
            fold(game.croncher.direction.ordinal.toLong())
    ...
    fun `frightened ghosts wander differently under a different seed`() {
        ...
        fun wander(seed: Long): String {
            ...
            // Everyone out on the maze and scared; the croncher parked out of the way in
            ...
            repeat(200) {
                g.croncher.placeAtTileCentre(Maze.CRONCHER_START_TILE, Direction.UP)
                g.tick()
            }
    ...
    fun `croncher never ends up inside a wall`() {
        ...
        repeat(20_000) { tick ->
            ...
            game.tick()
            if (game.phase == GamePhase.GAME_OVER) game.startNewGame()
            val t = game.croncher.tile()
            assertTrue("croncher is inside a wall at $t", game.maze.isWalkable(t.x, t.y))
        }
    }
