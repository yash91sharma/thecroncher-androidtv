...
/**
 * What happens when Pac-Man and a ghost occupy the same tile — which, depending on
...
class CollisionTest {
    ...
    /** Drops a ghost straight onto Pac-Man. */
    private fun GameState.collide(kind: GhostKind) {
        val g = ghost(kind)
        g.placeAtTileCentre(pacman.tile(), Direction.LEFT)
    }
    ...
    fun `a frightened ghost is eaten for two hundred`() {
        ...
        g.pacman.placeAtTileCentre(spot, Direction.DOWN)
    ...
    fun `eating four in one energizer pays 200 400 800 1600`() {
        ...
        g.pacman.placeAtTileCentre(g.maze.energizerPositions.first(), Direction.DOWN)
        g.tick()
        ...
        val pocket = Maze.PACMAN_START_TILE

        val values = listOf(200, 400, 800, 1600)
        for ((i, kind) in GhostKind.entries.withIndex()) {
            g.pacman.placeAtTileCentre(pocket, Direction.UP)
    ...
    fun `ghosts still in the house cannot catch anyone`() {
        ...
        clyde.placeAtTileCentre(g.pacman.tile(), Direction.LEFT)
        g.tick()
        assertEquals(livesBefore, g.lives)
    }
