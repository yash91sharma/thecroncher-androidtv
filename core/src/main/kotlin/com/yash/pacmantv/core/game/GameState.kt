...
enum class GamePhase {
    ...
    /** Pac-Man has been caught and is playing his death animation. */
    DYING,
...
class GameState(
    ...
    var fruitTile: TilePos? = null
        private set
    private var fruitTicksRemaining = 0
    private var fruitsShownThisLevel = 0
    ...
    private fun placeEveryoneForNewLife() {
        pacman.reset(speedOf(spec.pacmanSpeed))
        for (g in ghosts) {
            g.reset()
    ...
    fun requestDirection(dir: Direction) = pacman.requestDirection(dir)
    ...
    private fun tickPlaying() {
        ...
        // Checked either side of the ghosts' move: without this a ghost and
        // Pac-Man can swap tiles in one tick and pass straight through each other.
    ...
    private fun eatWhateverPacmanIsStandingOn() {
        val tile = pacman.tile()
    ...
    private fun maybeReleaseFruit() {
        if (fruitsShownThisLevel >= FRUIT_APPEARANCES.size) return
        if (dotsEaten != FRUIT_APPEARANCES[fruitsShownThisLevel]) return
        fruitsShownThisLevel++
        fruitTile = FRUIT_TILE
        fruitTicksRemaining = FRUIT_TICKS
    }

    private fun tickFruit() {
        val tile = fruitTile ?: return
        if (pacman.tile() == tile) {
            scores.eatFruit(LevelTable.fruitForLevel(level))
            audio.play(SoundEvent.FRUIT_EATEN)
            fruitTile = null
            return
        }
        if (--fruitTicksRemaining <= 0) fruitTile = null
    }
    ...
    companion object {
        ...
        /** Fruit appears after this many dots, twice per level. */
        val FRUIT_APPEARANCES = intArrayOf(70, 170)

        /** It sits just below the ghost house. */
        val FRUIT_TILE = TilePos(13, 17)

        const val FRUIT_TICKS = 9 * TICKS_PER_SECOND
    }
}
