...
object GhostAi {
    ...
    fun chaseTarget(
        ...
        // Straight at him.
        GhostKind.BLINKY -> pacmanTile

        // Four tiles in front, to cut him off.
        GhostKind.PINKY -> ahead(pacmanTile, pacmanDirection, 4)

        // The vector from Blinky to two-ahead-of-Pac-Man, doubled — which is why
        // Inky is only dangerous when Blinky is close.
        GhostKind.INKY -> {
            val pivot = ahead(pacmanTile, pacmanDirection, 2)
            TilePos(2 * pivot.x - blinkyTile.x, 2 * pivot.y - blinkyTile.y)
        }

        // Bold at a distance, shy up close.
        GhostKind.CLYDE ->
            if (ghostTile.squaredDistanceTo(pacmanTile) > CLYDE_PANIC_DISTANCE_SQUARED) {
                pacmanTile
            } else {
                Maze.SCATTER_CLYDE
            }
    ...
    /**
     * [distance] tiles in front of Pac-Man — reproducing the original overflow
