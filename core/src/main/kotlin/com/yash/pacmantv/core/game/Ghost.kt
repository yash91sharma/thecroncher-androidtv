...
class Ghost(
    ...
    /** Ghosts may use the door and the house; Pac-Man may not. */
    ...
    private fun currentTarget(ctx: GhostContext): TilePos = when (mode) {
        GhostMode.SCATTER -> GhostAi.scatterTarget(kind)
            ...
            kind = kind,
            pacmanTile = ctx.pacmanTile,
            pacmanDirection = ctx.pacmanDirection,
    ...
    data class GhostContext(
        val pacmanTile: TilePos,
        val pacmanDirection: Direction,
