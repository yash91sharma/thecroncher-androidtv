...
data class LevelSpec(
    val croncherSpeed: Double,
    val croncherFrightSpeed: Double,
...
object LevelTable {
    ...
    private val LEVELS: List<LevelSpec> = listOf(
        //             cat   catFr ghost tunnel gFright  fright flashes elroy
    ...
    private val TOY_BY_LEVEL = listOf(
        Toy.YARN, Toy.MILK, Toy.FISH, Toy.FISH,
        Toy.MOUSE, Toy.MOUSE, Toy.FEATHER, Toy.FEATHER,
        Toy.BIRD, Toy.BIRD, Toy.BELL, Toy.BELL,
        Toy.GOLDFISH,
    )

    /** From level thirteen onwards it is always the golden fish. */
    fun toyForLevel(level: Int): Toy =
        TOY_BY_LEVEL[(level - 1).coerceIn(0, TOY_BY_LEVEL.size - 1)]
}
